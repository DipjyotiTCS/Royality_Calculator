...
def calculateSales(request: Request):
  ...
  # Convert row to SalesData object
    ...
    #Perform calculation
    canada_old_amount = sales_data.sales_canada * request.royalty_table_canada
    canada_new_amount = sales_data.sales_canada * request.royalty_latest_canada
    canada_diff = 0 if canada_old_amount == canada_new_amount else (canada_new_amount - canada_old_amount)
    
    chapter_old_amount = sales_data.sales_chapter * request.royalty_table_chapter
    chapter_new_amount = sales_data.sales_chapter * request.royalty_latest_chapter
    chapter_diff = 0 if chapter_old_amount == chapter_new_amount else (chapter_new_amount - chapter_old_amount)
    
    us_old_amount = sales_data.sales_us * request.royalty_table_us
    us_new_amount = sales_data.sales_us * request.royalty_latest_us
    us_diff = 0 if us_old_amount == us_new_amount else (us_new_amount - us_old_amount)
    
    foreign_old_amount = sales_data.sales_foreign * request.royalty_table_foreign
    foreign_new_amount = sales_data.sales_foreign * request.royalty_latest_foreign
    foreign_diff = 0 if foreign_old_amount == foreign_new_amount else (foreign_new_amount - foreign_old_amount)
    
    
    highDiscout_old_amount = sales_data.sales_high_discount * request.royalty_table_high_discount
    highDiscout_new_amount = sales_data.sales_high_discount * request.royalty_latest_high_discount
    highDiscout_diff = 0 if highDiscout_old_amount == highDiscout_new_amount else (highDiscout_new_amount - highDiscout_old_amount)
    
    
    stateAdoption_old_amount = sales_data.sales_state_adoption * request.royalty_table_state_adoption
    stateAdoption_new_amount = sales_data.sales_state_adoption * request.royalty_latest_state_adoption
    ...
    subUs_old_amount = sales_data.sales_sub_us * request.royalty_table_sub_us
    subUs_new_amount = sales_data.sales_sub_us * request.royalty_latest_sub_us
    subUs_diff = 0 if subUs_old_amount == subUs_new_amount else (subUs_new_amount - subUs_old_amount)
    
    subForeign_old_amount = sales_data.sales_sub_foreign * request.royalty_table_sub_foreign
    subForeign_new_amount = sales_data.sales_sub_foreign * request.royalty_latest_sub_foreign
    subForeign_diff = 0 if subForeign_old_amount == subForeign_new_amount else (subForeign_new_amount - subForeign_old_amount)
    
    subTrial_old_amount = sales_data.sales_sub_trial * request.royalty_table_sub_trial
    subTrial_new_amount = sales_data.sales_sub_trial * request.royalty_latest_sub_trial
